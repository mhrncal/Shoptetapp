...
class DiagController extends BaseController
{
    ...
    public function testHeureka(): void
    {
        ...
        echo "cURL error: " . ($err ?: 'žádná') . "\n";
        echo "Body length: " . strlen($body) . "\n";
        echo "Body preview: " . substr($body, 0, 300) . "\n";
        echo "---\n";

        if (!$body || $code !== 200) { echo "CHYBA: prázdná odpověď nebo špatný HTTP kód\n"; exit; }
        ...
        if (!$doc) {
            echo "CHYBA: simplexml_load_string selhalo\n";
            foreach ($errs as $e) echo "  XML error: " . trim($e->message) . "\n";
            exit;
        }

        $count = count($doc->review);
        echo "XML OK, počet <review>: $count\n";
        if ($count > 0) {
            $r = $doc->review[0];
            echo "První recenze:\n";
            echo "  rating_id: " . $r->rating_id . "\n";
            echo "  total_rating: " . $r->total_rating . "\n";
            echo "  unix_timestamp: " . $r->unix_timestamp . "\n";
            echo "  pros: " . substr((string)$r->pros, 0, 100) . "\n";
            echo "  summary: " . substr((string)$r->summary, 0, 100) . "\n";
        }
    }
}
