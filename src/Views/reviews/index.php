...
<!-- Hlavička: název vlevo, exporty pod ním na mobilu -->
<div class="mb-3">
    <div class="d-flex justify-content-between align-items-center gap-2 mb-2">
        ...
        <!-- Na desktopu exporty vpravo od názvu -->
        <div class="d-none d-sm-flex gap-2 flex-shrink-0">
            ...
            <div class="dropdown">
                <button class="btn btn-sm btn-outline-info dropdown-toggle" type="button" data-bs-toggle="dropdown">
                    <i class="bi bi-link-45deg me-1"></i>Feed URL
                </button>
                <ul class="dropdown-menu dropdown-menu-end" style="min-width:320px;">
                    <li class="px-3 py-2">
                        <small class="text-muted d-block mb-2"><i class="bi bi-clock me-1"></i>Denně v 18:00</small>
    ...
    <!-- Na mobilu exporty jako celý řádek pod názvem -->
    <div class="d-flex d-sm-none gap-2">
        <a href="/reviews/export/csv" class="btn btn-sm btn-outline-primary flex-grow-1">
            <i class="bi bi-file-earmark-spreadsheet me-1"></i>CSV
        </a>
        <a href="/reviews/export/xml" class="btn btn-sm btn-outline-success flex-grow-1">
            <i class="bi bi-file-earmark-code me-1"></i>XML
        </a>
        <?php if (!empty($xmlFeedUrl)): ?>
        <button class="btn btn-sm btn-outline-info flex-grow-1" type="button"
                onclick="navigator.clipboard.writeText('<?= $e($xmlFeedUrl) ?>');this.innerHTML='<i class=\'bi bi-check me-1\'></i>Zkopírováno'">
            <i class="bi bi-link-45deg me-1"></i>Feed URL
        </button>
        <?php endif; ?>
    </div>
...
<!-- Status filtry — scrollovatelné bez zalomení -->
<div class="mb-3" style="overflow-x:auto; -webkit-overflow-scrolling:touch; scrollbar-width:none;">
    <div class="d-flex gap-2 pb-1" style="flex-wrap:nowrap; width:max-content;">
        <?php $tabs = [
...
<div class="card mb-3">
    <div class="card-body py-2 px-3">
        <form method="GET" class="d-flex gap-2">
            ...
            <button type="submit" class="btn btn-primary btn-sm px-3">Hledat</button>
            <?php if ($search): ?>
            <a href="?status=<?= $e($status) ?>" class="btn btn-outline-secondary btn-sm">✕</a>
            <?php endif; ?>
        </form>
    </div>
</div>
...
<form method="POST" action="/reviews/bulk" id="bulkForm">
    ...
    <!-- Hromadné akce — kompaktní na mobilu -->
    <div class="card mb-2">
        <div class="card-body py-2 px-3">
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <label class="d-flex align-items-center gap-2 mb-0 flex-shrink-0">
                    ...
                    <span class="small text-muted">Vše</span>
                </label>
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                ...
                                <form method="POST" action="/reviews/change-status">
                                    ...
                                    <button class="btn btn-sm btn-outline-success" title="Schválit"><i class="bi bi-check-lg"></i></button>
                                </form>
                                <?php endif; ?>
                                ...
                                <form method="POST" action="/reviews/change-status">
                                    ...
                                    <button class="btn btn-sm btn-outline-danger" title="Zamítnout"><i class="bi bi-x-lg"></i></button>
                                </form>
                                <?php endif; ?>
                                <form method="POST" action="/reviews/delete" onsubmit="return confirm('Smazat recenzi?')">
                                    <input type="hidden" name="_csrf" value="<?= $csrfToken ?? '' ?>">
                                    <input type="hidden" name="id" value="<?= $r['id'] ?>">
                                    <button class="btn btn-sm btn-outline-danger" title="Smazat"><i class="bi bi-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="card-body py-3 px-3">
                ...
                <div class="d-flex align-items-center gap-2 mb-1">
                    <input class="form-check-input review-cb flex-shrink-0" type="checkbox"
                           name="ids[]" value="<?= $r['id'] ?>">
                    <div class="fw-semibold text-truncate flex-grow-1"><?= $e($r['author_name']) ?></div>
                ...
                <!-- Řádek 3: meta (SKU, fotky, datum) -->
                <div class="d-flex flex-wrap gap-2 mb-3 ps-4" style="font-size:.8rem;">
                    <?php if ($r['sku']): ?>
                    <span class="text-muted"><i class="bi bi-upc me-1"></i><code class="text-primary" style="font-size:.78rem;"><?= $e($r['sku']) ?></code></span>
                    <?php endif; ?>
                    <?php if ($r['photo_count'] > 0): ?>
                    <span class="text-muted"><i class="bi bi-images me-1"></i><?= $r['photo_count'] ?> foto</span>
                    <?php endif; ?>
                    <span class="text-muted"><i class="bi bi-clock me-1"></i><?= date('d.m.Y', strtotime($r['created_at'])) ?></span>
                    <?php if ($r['imported']): ?>
                    <span class="text-success"><i class="bi bi-box-arrow-in-down me-1"></i>Importováno</span>
                    <?php endif; ?>
                </div>

                <!-- Řádek 4: akce -->
                <div class="d-flex gap-2">
                    <a href="/reviews/<?= $r['id'] ?>" class="btn btn-sm btn-outline-secondary" style="min-width:44px;">
                        <i class="bi bi-eye"></i>
                    </a>
                    <?php if ($r['status'] !== 'approved'): ?>
                    ...
                    <?php endif; ?>
                    <form method="POST" action="/reviews/delete" onsubmit="return confirm('Smazat recenzi?')" style="flex-shrink:0;">
                        <input type="hidden" name="_csrf" value="<?= $csrfToken ?? '' ?>">
                        <input type="hidden" name="id" value="<?= $r['id'] ?>">
                        <button class="btn btn-sm btn-outline-danger" style="min-width:44px;"><i class="bi bi-trash"></i></button>
                    </form>
                </div>
...
<nav class="mt-3">
    <ul class="pagination pagination-sm justify-content-center mb-0 flex-wrap">
        ...
        ?>
        <?php if ($page > 1): ?>
        <li class="page-item"><a class="page-link" href="?page=<?= $page-1 ?>&status=<?= $e($status) ?>&search=<?= $e($search) ?>">‹</a></li>
        <?php endif; ?>
